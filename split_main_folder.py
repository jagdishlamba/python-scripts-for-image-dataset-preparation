############################################################
# python3 move.py /absolute_path_of_folder_need to split/     To use this py file
#############################################################
import argparse
import os
import shutil
##############################################
# N is reqd number to be splitted
############################################
...
def move_files(abs_dirname):
    ...
    for f in files:
        ...
        if i % N == 0:
            subdir_name = os.path.join(abs_dirname, '{0:01}'.format(i / N + 1))
